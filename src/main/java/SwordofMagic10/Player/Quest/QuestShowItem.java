...
public class QuestShowItem extends QuestPhase {
    private final List<SomItemStack> reqItem = new ArrayList<>();
    private String handler;

    public QuestShowItem(QuestData questData) {
        super(questData);
    }
    ...
    public boolean check(PlayerData playerData) {
        for (SomItemStack reqItem : reqItem) {
            if (!playerData.getItemInventory().req(reqItem, 1)) {
                return false;
            }
        }
        return true;
    ...
    public boolean showItem(PlayerData playerData) {
        if (!flag) {
            if (check(playerData)) {
                flag = true;
                playerData.sendMessage("§c要求§aされた§eアイテム§aを見せました", SomSound.Tick);
                return true;
            } else {
            ...
            playerData.sendMessage("§aすでに§eアイテム§aを見せました", SomSound.Nope);
            return false;
        }
    }
