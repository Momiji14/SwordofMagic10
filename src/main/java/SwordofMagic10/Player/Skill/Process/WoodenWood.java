...
public class WoodenWood extends SomSkill {
    public WoodenWood(PlayerData playerData) {
        super(playerData);
    }
    ...
    public String active() {
        SomEffect effect = new SomEffect(this, false).setTime(1).setSilence("All");
        ...
        if(playerData.hasBottle(bottle2)){
            for (DurationSkill.MagicCircle magicCircle : ((Symbol) playerData.getSkillManager().getSkill("Symbol")).getSymbols()){
                ...
                durationSkill.setRunnable(()->{
                    ...
                    for(SomEntity entity : SomEntity.nearSomEntity(playerData.getTargets(), locations, finalRadius)){
