...
public class ReqClassExp implements SomCommand {
    ...
    public boolean PlayerCommand(Player player, PlayerData playerData, String[] args) {
        if (args.length == 1) {
            try {
                ...
                playerData.sendMessage("§eLv" + level + "§7: §a" + scale(Classes.getReqExp(level)));
            } catch (Exception ignore) {}
        } else {
            for (int i = 1; i <= Classes.MaxLevel; i++) {
                playerData.sendMessage("§eLv" + i + "§7: §a" + scale(Classes.getReqExp(i)));
            }
        }
        return true;
    }
